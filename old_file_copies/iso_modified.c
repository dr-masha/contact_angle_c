	...
	if ((v1 >= 0. && v2 < 0.) || (v1 < 0. && v2 >= 0.))
	{
		...
		by extrapolation from slices below */
	  {
	     ...
	     else
	     {
          ...
          if(INTERPOLATE) fluid_value = rho*ff1[i][j] + (1-rho)*ff2[i][j];
	 ...
	 if ( vertices[1][i][j].orientation == RIGHT )  
		 ...
		 by extrapolation  "upper" x-values */
	  {
	     ...
	     else
	     {
          ...
          if(INTERPOLATE) fluid_value = rho*ff1[i][j] + (1-rho)*ff1[i+1][j];
	 ...
	 if ( vertices[2][i][j].orientation == RIGHT ) 
	         ...
	         extrapolation from "upper" y-values */
	  {
	     ...
	     else
	     {
          ...
          if(INTERPOLATE) fluid_value = rho*ff1[i][j] + (1-rho)*ff1[i][j+1];
