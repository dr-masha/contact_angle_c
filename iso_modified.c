...
static void ** malloc2D_copy (guint nx, guint ny, gulong size)
{
...
static void free2D_copy (void ** m, guint nx)
{
  guint i;
 ...
 */
GtsIsoSlice * gts_iso_slice_new_copy (guint nx, guint ny)
{
  GtsIsoSlice * slice;
  ...
  slice->vertices[0] = 
    (OrientedVertex **) malloc2D_copy (nx, ny, sizeof (OrientedVertex));
  slice->vertices[1] = 
    (OrientedVertex **) malloc2D_copy  (nx - 1, ny, sizeof (OrientedVertex));
  slice->vertices[2] = 
    (OrientedVertex **) malloc2D_copy  (nx, ny - 1, sizeof (OrientedVertex));
  slice->nx = nx;
  slice->ny = ny;
 ...
 */
void gts_iso_slice_fill_cartesian_copy (GtsIsoSlice * slice,
				   GtsCartesianGrid g,
				   gdouble ** f1,
				   gdouble ** f2,
				   gdouble iso,
				   GtsVertexClass * klass)
{
  ...
  for (i = 0, x = g.x; i < g.nx - 1; i++, x += g.dx)
	...
	zz = g.z;
	vertices[1][i][j].v = 
	  gts_vertex_new (klass, xx, yy, zz);
	vertices[1][i][j].orientation = v2 >= 0. ? RIGHT : LEFT;
        printf("\n%g %g %g -- i=%d v1 %g v2 %g",xx,yy,zz,i,v1,v2);
      }
      else
  ...
  for (i = 0, x = g.x; i < g.nx; i++, x += g.dx)
	...
	printf("\n%g %g %g -- j=%d v1 %g v2 %g",xx,yy,zz,j,v1,v2);
      }
      else
 ...
 */
void gts_iso_slice_destroy_copy (GtsIsoSlice * slice)
{
  g_return_if_fail (slice != NULL);

  free2D_copy  ((void **) slice->vertices[0], slice->nx);
  free2D_copy  ((void **) slice->vertices[1], slice->nx - 1);
  free2D_copy  ((void **) slice->vertices[2], slice->nx);  
 ...
 */
void gts_isosurface_slice_copy (GtsIsoSlice * slice1,
			   GtsIsoSlice * slice2,
			   GtsSurface * surface)
{
  guint j, k, l, nx, ny;
 ...
 */
void gts_isosurface_cartesian_copy (GtsSurface * surface,
			       GtsCartesianGrid g,
			       ...
			       gdouble iso)
{
  ...
  slice1 = gts_iso_slice_new_copy (g.nx, g.ny);
  slice2 = gts_iso_slice_new_copy (g.nx, g.ny);
  f1 = (gdouble **) malloc2D_copy (g.nx, g.ny, sizeof (gdouble));
  f2 = (gdouble **) malloc2D_copy (g.nx, g.ny, sizeof (gdouble));

  (*f) (f1, g, 0, data);
  g.z += g.dz;
  (*f) (f2, g, 1, data);
  g.z -= g.dz;
  gts_iso_slice_fill_cartesian_copy (slice1, g, f1, f2, iso, 
  ...
  for (i = 2; i < g.nz; i++) {
    ...
    SWAP (f1, f2, tmp);
    g.z -= g.dz;
    gts_iso_slice_fill_cartesian_copy (slice2, g, f1, f2, iso, 
				  surface->vertex_class);
    g.z += g.dz;
    gts_isosurface_slice_copy (slice1, slice2, surface);
    SWAP (slice1, slice2, tmp);
  }
  gts_iso_slice_fill_cartesian_copy (slice2, g, f2, NULL, iso,
				surface->vertex_class);
  gts_isosurface_slice_copy (slice1, slice2, surface);

  gts_iso_slice_destroy_copy (slice1);
  gts_iso_slice_destroy_copy (slice2);
  free2D_copy ((void **) f1, g.nx);
  free2D_copy ((void **) f2, g.nx);
}
